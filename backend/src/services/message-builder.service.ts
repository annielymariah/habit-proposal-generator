...
export class MessageBuilderService {
    ...
    public getMessage(requestBody: RequestBody): string {
        ...
        const message = 
            `*PROPOSTA DE TRABALHO PARA ${messageVariables.jobComplement.toUpperCase()} EM ${messageVariables.city.split(",").at(0)}:*
🔸Local: ${messageVariables.city.toUpperCase()}.
🔸Função: ${messageVariables.jobComplement}.
🔸Salário base de R$ ${messageVariables.baseSalary}.
🔸Modalidade ${messageVariables.contractType}.
