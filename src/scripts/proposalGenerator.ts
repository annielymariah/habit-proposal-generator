...
export function generateProposal(values: ProcessedFormValues): string {
  const salaryTable: Record<string, number> = {
    ...
    "Desenvolvedor Júnior": 1518,
    "Desenvolvedor Pleno": 2000,
    "Desenvolvedor Sênior": 2500,
    "Eletricista": 2469,
    "Encarregado": 3195,
    "Meio-Oficial": 1925,
    "Auxiliar": 1925,
  ...
  const getAdicionais = (job: string, jobComplement?: string) => {
    ...
    jobLower.includes("meio-oficial") || jobLower.includes("ajudante") || jobLower.includes("profissional")) && (!complementLower.includes("mecanico") || !complementLower.includes("almoxarifado"))) {
